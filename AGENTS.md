...
- `lib/dreamdex.ts` — SDK wrapper: `listLiveMarkets()` (5s-pollable; hits `/api/markets` with a 15s abort, falls back to a direct browser `loadMarkets(true)`), `watchBook`/`watchFills` (WS pump-loops returning cancel fns), types. Client singleton.
- **`ensureRegistryLoaded()` before any symbol watch.** `watchOrderBook`/`watchTrades`/`fetchOrderBook` resolve a symbol against the SDK client's own market registry, so the browser client must have run `loadMarkets()` first. Before the `/api/markets` cache existed the fallback happened to do this on every poll; once the route started succeeding, watches failed silently forever (the pump loops swallow the error and retry). `lib/dreamdex.ts` now warms the registry with a 30s TTL inside every watch loop. Never call a symbol-based SDK read without it.
- **Market polling rules.** The Somnia indexer is frequently slow (`loadMarkets(true)` can hit the SDK's 30s timeout). `getMarketUniverse()` is therefore stale-while-revalidate: a module-level `lastResponse` is served instantly (expired windows filtered out, `meta.stale`/`meta.ageMs` set) while one shared refresh runs in the background, and a failed refresh never replaces good data. Client pollers must (1) guard against overlapping refreshes with a ref, (2) only show a loading state when the list is still empty, and (3) keep the previous markets on error — never `setMarkets([])`. Resetting status to "loading" on every poll blanks a working UI for the 10–30s a refresh takes; that was the `/fleet` "no live markets" bug.
...
- `lib/wallet.ts` — browser wallet layer: EIP-6963 provider discovery (falls back to `window.ethereum`), silent reconnect via `eth_accounts`, `accountsChanged`/`chainChanged` subscriptions, and `ensureShannonChain`/`ensureWalletClientChain` which request `wallet_switchEthereumChain` and add the chain on 4902 instead of failing. Never hard-fail a wrong chain — always offer the switch. Verify with `npx tsx scripts/verify-wallet.ts` (injects a mock EIP-6963 wallet; needs the dev server, `VERIFY_PORT` overrides 3111).
- `lib/fleet-runner.ts` — module-level fleet runner (NOT React state): owns cats, running flag, bankroll, the 1s `tickFleet` loop, and all book/trade watches. It lives outside the route tree so cats keep paper-trading while the user is on `/board` or `/intel`; `FleetDeck` is a view over it via `useSyncExternalStore`. Watches are deduped by `marketId` (several cats on one window share one subscription) and are started only while `running` is true — stopping the fleet or dropping a cat releases them, which is also the fix for the old unmount leak. Persists `{ cats, running, bankroll }`; `hydrateFleet()` clears every open `sim.position` on page load (a position opened against a previous session's book cannot be honestly marked) and reports the count via `droppedPositions`.
